...
class HttpRequestFactory {
	...
	HttpRequestBase createHttpRequest(Request<?> request,
			ClientConfiguration clientConfiguration, HttpEntity previousEntity,
			ExecutionContext context) {
		URI endpoint = request.getEndpoint();
		String uri = endpoint.toString();
		if (request.getResourcePath() != null
				&& request.getResourcePath().length() > 0) {
		...
		if (request.getHttpMethod() == HttpMethodName.POST) {
			...
			if (request.getContent() == null && encodedParams != null) {
				...
				postMethod.setEntity(new RepeatableInputStreamRequestEntity(
						request));
			}
			...
			throw new AmazonClientException("Unknown HTTP method name: "
	...
	private void configureHeaders(HttpRequestBase httpRequest,
			Request<?> request, ExecutionContext context,
		...
		if (context != null && context.getContextUserAgent() != null) {
			httpRequest.addHeader(
					"User-Agent",
					createUserAgentString(clientConfiguration,
							context.getContextUserAgent()));
		}
	}
	...
	private HttpEntity newStringEntity(String s) {
		try {
			...
			throw new AmazonClientException("Unable to create HTTP entity: "
					+ e.getMessage(), e);
		}
	}
	 ...
	 * errors as an AmazonClientException.
	 * 
	 * @param entity

...
import com.amazon.s3.util.HttpUtils;
import com.amazon.s3.util.StringInputStream;
...
public abstract class AbstractAWSSigner implements Signer {
	...
	protected String signAndBase64Encode(String data, String key,
		...
		try {
			return signAndBase64Encode(data.getBytes(DEFAULT_ENCODING), key,
					algorithm);
	...
	protected String signAndBase64Encode(byte[] data, String key,
			SigningAlgorithm algorithm) throws AmazonClientException {
		try {
			byte[] signature = sign(data, key.getBytes(DEFAULT_ENCODING),
					algorithm);
	...
	protected byte[] hash(InputStream input) throws AmazonClientException {
		try {
			MessageDigest md = MessageDigest.getInstance("SHA-256");
			DigestInputStream digestInputStream = new DigestInputStream(input,
					md);
	...
	protected String getCanonicalizedQueryString(Map<String, String> parameters) {
		...
		while (pairs.hasNext()) {
			...
			sorted.put(HttpUtils.urlEncode(key, false),
					HttpUtils.urlEncode(value, false));
		}
	...
	protected byte[] getBinaryRequestPayload(Request<?> request) {
		if (HttpUtils.usePayloadForQueryParameters(request)) {
			...
			try {
				...
				throw new AmazonClientException(
						"Unable to encode string into bytes");
			}
		}
	 ...
	 *         form encoding of query string params.
	 */
	protected byte[] getBinaryRequestPayloadWithoutQueryParams(
			Request<?> request) {
		...
		if (HttpUtils.usePayloadForQueryParameters(request)) {
			...
			try {
				return new ByteArrayInputStream(
						encodedParameters.getBytes(DEFAULT_ENCODING));
			} catch (UnsupportedEncodingException e) {
				throw new AmazonClientException(
						"Unable to encode string into bytes");
			}
	 ...
	 * 
	 * @param bytes
	 *            UTF-8 encoded binary character data.
	 * 
	...
	protected String newString(byte[] bytes) {
		try {
			...
			throw new AmazonClientException("Unable to encode bytes to String",
					e);
		}
	}

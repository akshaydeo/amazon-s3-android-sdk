...
public class ChecksumValidatingInputStream extends FilterInputStream {
	...
	/**
	 * Description of the source object from Amazon S3 in case we need to alert
	 * the user that the checksums didn't match.
	 */
	 ...
	 * Amazon S3, then the close() method will throw an exception to alert the
	 * user about the likely data corruption.
	 * 
	 * @param in
	 ...
	 * @param sourceObject
	 *            A text description of the original source object in Amazon S3;
	 *            only used in an error message to give the user more context
	 *            about what failed if data corruption is detected.
	 * 
	 * @throws NoSuchAlgorithmException
	 *             If the running JVM doesn't support MD5 MessageDigests.
	 */
	public ChecksumValidatingInputStream(InputStream in,
			byte[] expectedChecksum, String sourceObject)
			throws NoSuchAlgorithmException {
		...
		if (!Arrays.equals(clientSideHash, expectedChecksum)) {
			throw new AmazonClientException(
					"Unable to verify integrity of data download.  "
							...
							+ "The data read from '" + sourceObject
							+ "' may be corrupt.");
		}
	}
}

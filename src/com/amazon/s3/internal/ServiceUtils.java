...
import com.amazon.s3.util.DateUtils;
...
public class ServiceUtils {
	...
	public static byte[] toByteArray(String s) {
		try {
			...
			Log.w(TAG, "Encoding " + Constants.DEFAULT_ENCODING
					+ " is not supported", e);
			return s.getBytes();
		}
	}
	...
	public static URL convertRequestToUrl(Request<?> request) {
		...
		try {
			...
			throw new AmazonClientException(
					"Unable to convert request to well formed URL: "
							+ e.getMessage(), e);
		}
	}
	 ...
	 *            Boolean valuable to indicate whether do the integrity check or
	 *            not
	 * 
	 */
	public static void downloadObjectToFile(S3Object s3Object,
		...
		try {
			...
			try {
				...
				Log.w(TAG, "Couldn't abort stream", e);
			}
			throw new AmazonClientException(
					"Unable to store object contents to disk: "
							+ e.getMessage(), e);
		} finally {
			try {
				outputStream.close();
		...
		try {
			...
			// side
			if (ServiceUtils.isMultipartUploadETag(s3Object.getObjectMetadata()
					.getETag()) == false) {
				clientSideHash = Md5Utils.computeMD5Hash(new FileInputStream(
						destinationFile));
				serverSideHash = BinaryUtils.fromHex(s3Object
						.getObjectMetadata().getETag());
			}
		} catch (Exception e) {
			Log.w(TAG, "Unable to calculate MD5 hash to validate download: "
					+ e.getMessage(), e);
		}
